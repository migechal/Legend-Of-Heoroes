...
void Button::print(SDL_Surface* screen){
    this->update();
    std::cout << (Uint32)color.r << " "<< (Uint32)color.g << " "<< (Uint32)color.b << " "<< (Uint32)color.a << std::endl;
    SDL_Surface* button = SDL_CreateRGBSurface(0, w, h, 32, 0, 0, 0, 0);

    SDL_FillRect(button, NULL, SDL_MapRGBA(button->format, color.r, color.g, color.b, color.a));
